...
def convert_state_to_tensor(s_raw, player_perspective=1):
    ...
    s = torch.tensor(player_perspective * s_raw).unsqueeze(0).float() # because it only has 1 channel
    # I believe I have to unsqueese to be able to use conv layers
    return s
...
# AlphaGo Zero Loss
def alphago_zero_loss(model, policy_output, value_output, true_policy, true_value, c=0.):
